...
(() => {
  ...
  const attachImages = async (request, context) => {
    ...
    if (!input) throw new Error("ADAPTER_INCOMPATIBLE: Gemini attachment file input was not found");
    if (attachments.length > 1 && input.multiple === false) {
      throw new Error("INVALID_REQUEST: Gemini composer currently accepts one attachment per file input");
    }
    ...
    try {
      ...
      throw new Error("ADAPTER_INCOMPATIBLE: Gemini attachment file input rejected DataTransfer files");
    }
