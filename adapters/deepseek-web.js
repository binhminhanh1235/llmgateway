// llmgateway built-in DeepSeek Web adapter.
// Contract v1. Runs inside an authenticated chat.deepseek.com page through loopback CDP.
(() => {
  ...
  const defaults = {
    ...
    send: [
      "div.ds-button--primary.ds-button--filled.ds-button--circle[role='button']",
  ...
  const submitComposer = (context, composer) => {
    ...
    if (queryFirst(context, "login")) {
      throw new Error("LOGIN_REQUIRED: DeepSeek session expired");
    }

    // DeepSeek's composer handles Enter on keydown. Use this only when a usable send
    ...
    if (accepted === false) {
      throw new Error("ADAPTER_INCOMPATIBLE: DeepSeek Enter submit was rejected by the composer");
    }
  ...
  const selectModel = async (context) => {
    ...
    if (desiredNorm.includes("deepseekwebdefault")) return null;
    const label = queryFirst(context, "modelTrigger");
    if (!label) throw new Error("MODEL_PICKER_NOT_FOUND: DeepSeek model picker is not visible");
    ...
    if (!match) throw new Error("MODEL_NOT_FOUND: DeepSeek model not available in picker: " + desired);
    match.click();
    await sleep(300);
    return desired;
  };
  ...
  const startStreamJob = (request, context) => {
    const streamId = "deepseek_stream_" + Date.now() + "_" + Math.random().toString(36).slice(2);
    const state = {
      streamId,
      completionId: "chatcmpl_deepseek_web_" + Date.now(),
      model: request?.model || context?.model_label || "deepseek-web",
      request,
      context,
      answer: "",
      delivered: "",
    ...
    Promise.resolve().then(async () => {
      try {
        ...
        if (!composer) throw new Error("ADAPTER_INCOMPATIBLE: DeepSeek prompt composer disappeared");
        ...
        while (Date.now() - startedAt < Number(context?.response_timeout_ms || 180000)) {
          ...
          if (queryFirst(context, "login")) {
            throw new Error("LOGIN_REQUIRED: DeepSeek session expired while waiting for a response");
          }
        ...
        if (!answer) throw new Error("RESPONSE_TIMEOUT: DeepSeek did not produce a readable response");
  ...
  globalThis.__LLMGATEWAY_ADAPTER__ = {
    ...
    async probe(context) {
      ...
      const hostOk = await waitForExpectedHost("chat.deepseek.com", probeTimeoutMs);
      if (!hostOk) {
        const currentHost = String(location.hostname || "").trim() || "<empty>";
        return { ok: false, code: "wrong_page", message: "Expected chat.deepseek.com but found " + currentHost };
      }
      await waitFor(
      ...
      if (!composer) {
        return {
          ...
          message: loginVisible
            ? "DeepSeek login is required before the adapter can run."
            : "DeepSeek prompt composer was not found; the web UI may have changed."
        };
      }
      return {
        ok: true,
        code: "ready",
        message: "DeepSeek composer detected",
        page_signature: "deepseek-composer-v1"
      };
    },
    ...
    async chat(request, context) {
      ...
      const body = openAIResult(
        request,
        request?.model || context?.model_label || "deepseek-web",
        answer,
        "chatcmpl_deepseek_web_"
      );
      if (request?.stream) {
        return {
          status: 200,
