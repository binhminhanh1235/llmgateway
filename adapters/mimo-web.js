// llmgateway built-in MiMo Web adapter.
// Contract v1. Runs inside an authenticated aistudio.xiaomimimo.com page through loopback CDP.
(() => {
  const CONTRACT_VERSION = 1;
  const ADAPTER_VERSION = "2026.09.06.1";

  const defaults = {
    input: [
      "textarea[placeholder*='Ask me anything' i]",
      "textarea[placeholder*='Shift+Enter' i]",
      "textarea",
    ...
    modelTrigger: [
      ...
      "[role='button'][class*='model' i]",
      "[class*='model' i] [role='button']",
  ...
  const submitComposer = (context, composer) => {
    ...
    if (queryFirst(context, "login")) {
      throw new Error("LOGIN_REQUIRED: MiMo session expired");
    }

    // MiMo's composer handles Enter on keydown. Use this only when a usable send
    ...
    if (accepted === false) {
      throw new Error("ADAPTER_INCOMPATIBLE: MiMo Enter submit was rejected by the composer");
    }
  ...
  const selectModel = async (context) => {
    ...
    if (desiredNorm.includes("mimowebdefault")) return null;
    const label = queryFirst(context, "modelTrigger");
    if (!label) throw new Error("MODEL_PICKER_NOT_FOUND: MiMo model picker is not visible");
    ...
    if (!match) throw new Error("MODEL_NOT_FOUND: MiMo model not available in picker: " + desired);
    match.click();
    await sleep(300);
    return desired;
  };
  ...
  const startStreamJob = (request, context) => {
    const streamId = "mimo_stream_" + Date.now() + "_" + Math.random().toString(36).slice(2);
    const state = {
      streamId,
      completionId: "chatcmpl_mimo_web_" + Date.now(),
      model: request?.model || context?.model_label || "mimo-web",
      request,
      context,
      answer: "",
      delivered: "",
    ...
    Promise.resolve().then(async () => {
      try {
        ...
        if (!composer) throw new Error("ADAPTER_INCOMPATIBLE: MiMo prompt composer disappeared");
        ...
        while (Date.now() - startedAt < Number(context?.response_timeout_ms || 180000)) {
          ...
          if (queryFirst(context, "login")) {
            throw new Error("LOGIN_REQUIRED: MiMo session expired while waiting for a response");
          }
        ...
        if (!answer) throw new Error("RESPONSE_TIMEOUT: MiMo did not produce a readable response");
  ...
  globalThis.__LLMGATEWAY_ADAPTER__ = {
    ...
    async probe(context) {
      ...
      const hostOk = await waitForExpectedHost("aistudio.xiaomimimo.com", probeTimeoutMs);
      if (!hostOk) {
        const currentHost = String(location.hostname || "").trim() || "<empty>";
        return { ok: false, code: "wrong_page", message: "Expected aistudio.xiaomimimo.com but found " + currentHost };
      }
      await waitFor(
      ...
      if (!composer) {
        return {
          ...
          message: loginVisible
            ? "MiMo login is required before the adapter can run."
            : "MiMo prompt composer was not found; the web UI may have changed."
        };
      }
      return {
        ok: true,
        code: "ready",
        message: "MiMo composer detected",
        page_signature: "mimo-composer-v1"
      };
    },
    ...
    async chat(request, context) {
      ...
      const body = openAIResult(
        request,
        request?.model || context?.model_label || "mimo-web",
        answer,
        "chatcmpl_mimo_web_"
      );
      if (request?.stream) {
