...
RESPONSE_FILE_JSON=$(curl -fsS -X POST "$BASE_URL/v1/files" \
  -H "Authorization: Bearer ${LLMGATEWAY_API_KEY}" \
  -F 'purpose=vision' \
  -F "file=@$TMP_DIR/vision.png;type=image/png")
RESPONSE_FILE_ID=$(printf '%s' "$RESPONSE_FILE_JSON" | python3 -c 'import json,sys; print(json.load(sys.stdin)["id"])')

python3 - "$RESPONSE_FILE_ID" "$TMP_DIR/responses.json" "$TMP_DIR/text-only.json" <<'PY'
import json,sys
file_id=sys.argv[1]
responses={
  "model":"llmgateway-auto",
...
# Responses persist gateway artifact references. The response-scoped image must
# remain protected independently of any later thread lifecycle.
STATUS=$(curl -sS -o "$TMP_DIR/error.json" -w '%{http_code}' -X DELETE "$BASE_URL/v1/files/$RESPONSE_FILE_ID" \
  -H "Authorization: Bearer ${LLMGATEWAY_API_KEY}")
assert_json_error 409 artifact_in_use "$STATUS"

THREAD_FILE_JSON=$(curl -fsS -X POST "$BASE_URL/v1/files" \
  -H "Authorization: Bearer ${LLMGATEWAY_API_KEY}" \
  -F 'purpose=vision' \
  -F "file=@$TMP_DIR/vision.png;type=image/png")
THREAD_FILE_ID=$(printf '%s' "$THREAD_FILE_JSON" | python3 -c 'import json,sys; print(json.load(sys.stdin)["id"])')
test "$THREAD_FILE_ID" != "$RESPONSE_FILE_ID"
...
python3 - "$THREAD_FILE_ID" "$TMP_DIR/thread-message.json" <<'PY'
import json,sys
body={
  "content":[
...
python3 - "$THREAD_FILE_ID" "$TMP_DIR/thread-detail.json" <<'PY'
import json,sys
file_id=sys.argv[1]
...
STATUS=$(curl -sS -o "$TMP_DIR/error.json" -w '%{http_code}' -X DELETE "$BASE_URL/v1/files/$THREAD_FILE_ID" \
  -H "Authorization: Bearer ${LLMGATEWAY_API_KEY}")
assert_json_error 409 artifact_in_use "$STATUS"

curl -fsS -X DELETE "$BASE_URL/v1/threads/$THREAD_ID" \
  -H "Authorization: Bearer ${LLMGATEWAY_API_KEY}" | grep -q '"deleted":true'
curl -fsS -X DELETE "$BASE_URL/v1/files/$THREAD_FILE_ID" \
  -H "Authorization: Bearer ${LLMGATEWAY_API_KEY}" | grep -q '"deleted":true'
