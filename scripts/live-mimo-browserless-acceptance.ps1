...
function Write-Step([string]$Message) {
    Write-Host "[mimo-browserless-live] $Message"
}
...
function Assert-DirectReady([object]$Runtime) {
    ...
    Assert-True (
        [string]$Runtime.adapter.adapter_id -eq "mimo-web-http"
...
function Assert-DirectExecution([object]$Runtime, [string]$Phase) {
    ...
    Assert-True (
        [string]$Runtime.last_execution.adapter_id -eq "mimo-web-http"
...
try {
    ...
    Assert-True ([string]$runtime.provider_kind -eq "browser-mimo") "account is not a Xiaomi MiMo browser provider"
    Assert-True ([string]$runtime.adapter.adapter_id -eq "mimo-web-http") "runtime selected an unexpected direct adapter"

    Write-Step "Refreshing MiMo Studio model catalog with Chromium stopped"
    $null = Invoke-GatewayJson -Method POST -Path "/_llmgateway/accounts/$AccountId/models/refresh"
    $runtime = Get-Runtime
    Assert-True (-not [bool]$runtime.browser_running) "Chromium started during MiMo model refresh"
    ...
    $threadB = New-TestThread "MiMo browserless acceptance B $(Get-Date -Format s)"
    ...
    Write-Host ""
    Write-Host "MIMO BROWSERLESS LIVE ACCEPTANCE: PASS" -ForegroundColor Green
