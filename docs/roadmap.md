...
- P6 Auth Generations & Gemini Hardening — **DONE / VERIFIED on `feat/provider-runtime-fabric` only** at `7a4592385141b3ad046f376995878813495c71a8`, tree `e83f96fe4311849b78b036b1fea4a46719c5de56`; CI #1954 / run `34104368716` PASS on Linux + Windows, including verified auth generations/fingerprints, stale-generation protection, valid-auth preservation during incomplete re-auth, strict Gemini auth readiness, typed 429/503 handling and bounded 40-request admission stress;
- P7 Logical Route / Transport Separation & Virtual-Model Continuity — **DONE / VERIFIED on `feat/provider-runtime-fabric` only** at `1f78c424501ae90c8fb817ee3b99ddc88359b16b`, tree `c36ecd4102094692ff6214b0620c739e31338059`; CI #2004 / run `34120630383` PASS on Linux + Windows, including AccountRuntime transport ownership, direct/warm/cold activation cost, canonical local context across provider/account failover, `llmgateway-best` Gemini continuity, `llmgateway-coding` Qwen continuity and bounded all-candidates-unavailable behavior;
- P8 Chaos & Resource deterministic acceptance — **DONE / VERIFIED on the same code head**; fault coverage includes WAF, 429/503, auth missing, direct/CDP/page/browser failure, empty/dropped/committed streams, cancellation, concurrent cold start, reclaim and route exhaustion; runtime diagnostics include browser process/lease/launch/reclaim plus account queue/in-flight/concurrency/generation metrics;
- P8 authenticated live acceptance — **READY / PENDING**; `scripts/live-provider-runtime-fabric.sh` composes real authenticated Gemini/Qwen/DeepSeek acceptance and verifies no leaked browser/account runtime resources, but CI does not possess the user's real provider sessions and therefore does not claim this live gate as executed.

**P0-P7 and P8 deterministic/resource verification are complete on the working branch. Final authenticated P8 live acceptance is still required before the initiative can be called fully DONE / VERIFIED. Provider Runtime Fabric remains NOT ON MAIN and must not be described as shipped until explicit merge approval.**
